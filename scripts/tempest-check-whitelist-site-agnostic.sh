#!/bin/bash
# tempest-check-whitelist-site-agnostic: invoked via `!include-raw-escape:` from data/builder-macros.yaml.
# CHECK and WHITELIST are injected as env vars by the macro; CLOUD comes from the
# build environment. No `set`: faithful to the bash (no -e) the inline shebang gave,
# so RET=${PIPESTATUS[0]} and the cleanup/exit run as before.
